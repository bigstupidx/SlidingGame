...
public class CameraShake : MonoBehaviour
{
    ...
    IEnumerator Shake()
    {
        ...
        while (shakeDuration > 0.01f)
        {
            ...
            if (smooth)
            {
                Quaternion lerpedRotation = Quaternion.Lerp(transform.localRotation, transform.localRotation * Quaternion.Euler(rotationAmount), Time.deltaTime * smoothAmount);
                transform.localRotation = ( transform.localRotation * Quaternion.Inverse(lerpedRotation)) * transform.localRotation;
            }
            else
